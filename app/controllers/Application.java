...
public class Application extends Controller {
	...
	public static void index() {
		...
		if (userId != null) {
			...
			if (userType.equals("simple")) {
				SimpleUsers.infoCenter(Long.parseLong(userId));
			} else if (userType.equals("cssa")) {
	...
	static SimpleUser connectedSimple() {
		String userId = session.get("logged");
		return userId == null ? null : (SimpleUser) SimpleUser.findById(Long.parseLong(userId));
	}

	static CSSA connectedCSSA() {
		String userId = session.get("logged");
		return userId == null ? null : (CSSA) CSSA.findById(Long.parseLong(userId));
	}
	...
	static void checkSecure() {
		...
		if (secure != null) {
			if (connectedSimple() == null || (secure.admin() && !connectedSimple().isAdmin())) {
				forbidden();
			}
		}
	}
